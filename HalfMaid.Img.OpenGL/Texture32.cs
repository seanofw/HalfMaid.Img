...
{
	/// <summary>
	/// An OpenGL 24-bit RGB or 32-bit RGBA texture.
	/// </summary>
	[DebuggerDisplay("{Name} ({Width}x{Height})")]
	public class Texture32 : IDisposable
	{
		#region Properties
			...
			set
			{
				if (_borderColor != value)
				{
					UpdateTexParam(() =>
					{
						...
						color[0] = _borderColor.Rf;
						color[1] = _borderColor.Gf;
						color[2] = _borderColor.Bf;
						color[3] = _borderColor.Af;
		...
		private Color32 _borderColor = default;
		...
		/// <summary>
		/// Whether this texture was created with an alpha channel (32 bit) or without (24 bit).
		/// </summary>
		...
		/// </summary>
		/// <param name="data">A pointer to the source data, in 32-bit RGBA format.  The source
		/// data can be presumed to have been copied into the texture, and can safely be disposed
		/// after this call.</param>
		...
		/// The default is to wrap to the other side.</param>
		/// <param name="includeAlpha">Whether to include an alpha channel (32 bit texture) or
		/// not (24 bit texture).  If this texture does not have an alpha channel, the alpha values
		/// will not be copied from the source data.</param>
		...
		public unsafe Texture32(IntPtr data, int width, int height, string? name,
			...
			bool includeAlpha = true, bool autoMipMap = false)
		{
			Width = width;
			Height = height;
			Name = name;
			...
			Handle = CreateOpenGLObject((void*)data, width, height, name,
				minFilter, magFilter, wrapS, wrapT, includeAlpha, autoMipMap);
		...
		/// </summary>
		/// <param name="data">An array of source color data, in 32-bit RGBA format.  The source
		/// data can be presumed to have been copied into the texture, and can safely be disposed
		/// after this call.</param>
		/// <param name="width">The width of the texture.</param>
		/// <param name="height">The height of the texture.</param>
		...
		/// <exception cref="ArgumentException">Thrown if the width or height are invalid.</exception>
		[MethodImpl(MethodImplOptions.NoInlining)]
		public unsafe Texture32(Color32[] data, int width, int height, string? name,
			TextureMinFilter minFilter = TextureMinFilter.Nearest, TextureMagFilter magFilter = TextureMagFilter.Nearest,
			TextureWrapMode wrapS = TextureWrapMode.Repeat, TextureWrapMode wrapT = TextureWrapMode.Repeat,
			bool includeAlpha = true, bool autoMipMap = false)
		{
		...
		public Texture32(Vector2i size, string? name = null,
			TextureMinFilter minFilter = TextureMinFilter.Nearest, TextureMagFilter magFilter = TextureMagFilter.Nearest,
			TextureWrapMode wrapS = TextureWrapMode.Repeat, TextureWrapMode wrapT = TextureWrapMode.Repeat,
			bool includeAlpha = true, bool autoMipMap = false)
			: this(IntPtr.Zero, size.X, size.Y, name, minFilter, magFilter, wrapS, wrapT, includeAlpha, autoMipMap)
		{
		}
		...
		/// The default is false.</param>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Texture32(int width, int height, string? name = null,
			TextureMinFilter minFilter = TextureMinFilter.Nearest, TextureMagFilter magFilter = TextureMagFilter.Nearest,
			TextureWrapMode wrapS = TextureWrapMode.Repeat, TextureWrapMode wrapT = TextureWrapMode.Repeat,
			bool includeAlpha = true, bool autoMipMap = false)
			: this(IntPtr.Zero, width, height, name, minFilter, magFilter, wrapS, wrapT, includeAlpha, autoMipMap)
		{
		}
		...
		/// <exception cref="ArgumentException">Thrown if the width or height are invalid.</exception>
		[MethodImpl(MethodImplOptions.NoInlining)]
		public Texture32(Color32[] data, Vector2i size, string? name,
			TextureMinFilter minFilter = TextureMinFilter.Nearest, TextureMagFilter magFilter = TextureMagFilter.Nearest,
			TextureWrapMode wrapS = TextureWrapMode.Repeat, TextureWrapMode wrapT = TextureWrapMode.Repeat,
			bool includeAlpha = true, bool autoMipMap = false)
			: this(data, size.X, size.Y, name, minFilter, magFilter, wrapS, wrapT, includeAlpha, autoMipMap)
		{
		}
		...
		/// </summary>
		/// <param name="image">A 32-bit RGBA format image to use to create the texture.  The source
		/// image can be presumed to have been copied into the texture, and can safely be disposed
		/// after this call.</param>
		/// <param name="minFilter">The minification filter to use when the texture is rendered small.
		/// The default is nearest-neighbor mapping, not any form of filtering.</param>
		/// <param name="magFilter">The magnification filter to use when the texture is rendered large.
		...
		/// <exception cref="ArgumentException">Thrown if the width or height are invalid.</exception>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Texture32(PureImage32 image, string? name = null,
			TextureMinFilter minFilter = TextureMinFilter.Nearest, TextureMagFilter magFilter = TextureMagFilter.Nearest,
			TextureWrapMode wrapS = TextureWrapMode.Repeat, TextureWrapMode wrapT = TextureWrapMode.Repeat,
			bool includeAlpha = true, bool autoMipMap = false)
			: this(image.Data, image.Width, image.Height, name, minFilter, magFilter, wrapS, wrapT, includeAlpha, autoMipMap)
		{
		}
		...
		private static unsafe int CreateOpenGLObject(void* data, int width, int height, string? name,
			...
			TextureWrapMode wrapS, TextureWrapMode wrapT, bool includeAlpha, bool autoMipMap)
		{
			if (width < 0 || height < 0)
			...
			try
			{
				...
				if (data != null)
				{
					GL.TexImage2D(TextureTarget.Texture2D, 0,
						includeAlpha ? PixelInternalFormat.Rgba8 : PixelInternalFormat.Rgb,
						width, height, 0,
		...
		public void Replace(PureImage32 image, int level = 0)
		...
		public void Replace(IntPtr srcData, int level = 0)
		{
			...
			GL.TexSubImage2D(TextureTarget.Texture2D, level,
				0, 0, Width, Height,
				PixelFormat.Rgba, PixelType.UnsignedByte, srcData);
		...
		/// </summary>
		/// <param name="srcData">The color data to copy into the texture.  This must be
		/// of the same width/height as the texture or an exception will be raised.</param>
		/// <param name="level">The mipmap level to update (0 by default).</param>
		/// <exception cref="ArgumentException">Thrown if the source data is too small for the texture.</exception>
		public void Replace(Color32[] srcData, int level = 0)
		{
		...
		/// <summary>
		/// Copy the given color data to the given target rectangle of this texture.
		/// </summary>
		...
		public void Blit(Color32[] srcData, Rect destRect, int level = 0)
		{
		...
		/// <param name="level">The mipmap level to update (0 by default).</param>
		public void Blit(PureImage32 srcImage, int destX, int destY, int level = 0)
		...
		/// <param name="level">The mipmap level to update (0 by default).</param>
		public void Blit(PureImage32 srcImage, int srcX, int srcY, int destX, int destY, int width, int height, int level = 0)
		{
			if (!Image32.ClipBlit(Size, srcImage.Size, ref srcX, ref srcY, ref destX, ref destY, ref width, ref height))
				return;

			PureImage32 subImage = (srcX != 0 || srcY != 0 || width != srcImage.Width || height != srcImage.Height
		...
		private void BlitInternal(Color32[] srcData, Rect destRect, int level)
		{
			RaisePriorErrors();
		...
		public void GetData(Color32[] buffer, int level = 0)
		{
			if (buffer.Length < Width * Height)
				throw new ArgumentException("Color buffer size must be as large as or larger than the texture.");
		...
		/// <summary>
		/// Copy this OpenGL Texture to a new Image object.
		/// </summary>
		/// <param name="level">The mipmap level to extract (0 by default).</param>
		/// <returns>A copy of this texture, as an Image.</returns>
		public Image32 ToImage(int level = 0)
		{
