...
namespace System.Collections.Generic
{
	...
	public class PriorityQueue<TElement, TPriority>
	{
		...
		public PriorityQueue(IEnumerable<(TElement Element, TPriority Priority)> items, IComparer<TPriority>? comparer)
		{
			...
			if (!(items is IReadOnlyCollection<(TElement Element, TPriority Priority)> collection))
			{
				List<(TElement Element, TPriority Priority)> list = new List<(TElement Element, TPriority Priority)>();
			...
			{
				_nodes = new (TElement Element, TPriority Priority)[collection.Count];
				int index = 0;
