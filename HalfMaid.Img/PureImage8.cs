...
namespace HalfMaid.Img
{
	...
	public readonly struct PureImage8 : IPureImage<byte>
	{
		...
		[Pure]
		public static PureImage8 LoadEmbeddedResource(Assembly assembly, string name)
			=> Image8.LoadEmbeddedResource(assembly, name) ?? Empty;
		...
		/// <returns>The new image, or PureImage8.Empty if it can't be loaded.</returns>
		[Pure]
		public static PureImage8 LoadFromBytes(ReadOnlySpan<byte> data, string? filenameIfKnown = null,
			ImageFormat imageFormat = default)
			=> Image8.LoadFromBytes(data, filenameIfKnown, imageFormat) ?? Empty;
		...
		/// </remarks>
		[Pure]
		public PureImage8 Blit(Image24 srcImage, int srcX, int srcY, int destX, int destY, int width, int height,
			BlitFlags blitFlags = default, byte color = default)
		{
			Image8 copy = _image.Clone();
			copy.Blit(srcImage, srcX, srcY, destX, destY, width, height, blitFlags, color);
		...
		/// </remarks>
		[Pure]
		public PureImage8 Blit(Image32 srcImage, int srcX, int srcY, int destX, int destY, int width, int height,
			BlitFlags blitFlags = default, byte color = default)
		{
			Image8 copy = _image.Clone();
			copy.Blit(srcImage, srcX, srcY, destX, destY, width, height, blitFlags, color);
		...
		public PureImage8 DrawMultilineText(Rectd rect, ReadOnlySpan<char> text, Font font,
			Color32 color, BlitFlags blitFlags = BlitFlags.ColorAlpha,
			TextAlignment textAlignment = default)
		{
			Image8 clone = _image.Clone();
		...
		/// <param name="blitFlags">Which blit mode to use when drawing each glyph.</param>
		/// <returns>The new image with the text drawn on it.</returns>
		public PureImage8 DrawText(Vector2d point, ReadOnlySpan<char> text, Font font,
			Color32 color, BlitFlags blitFlags = BlitFlags.ColorAlpha)
		{
			Image8 clone = _image.Clone();
		...
		/// <param name="blitFlags">Which blit mode to use when drawing each glyph.</param>
		/// <returns>The new image with the text drawn on it.</returns>
		public PureImage8 DrawText(Vector2d point, IEnumerable<PositionedGlyph> glyphs,
			Color32 color, BlitFlags blitFlags = BlitFlags.ColorAlpha)
		{
			Image8 clone = _image.Clone();
