...
namespace HalfMaid.Img
{
	...
	public sealed class Image32 : IImage<Color32>, IEquatable<Image32>
	{
		...
		public static Vector2i CalculateResize(Vector2i imageSize, int? userWidth, int? userHeight)
		{
			...
			if (userWidth.HasValue)
			{
				...
				{
					double ooAspectRatio = (double)imageSize.Y / imageSize.X;
			...
			{
				height = userHeight.Value;
				double aspectRatio = (double)imageSize.X / imageSize.Y;
		...
		/// </remarks>
		public Image32 Scale2x(PixelScaler pixelScaler, bool includeAlpha = true)
		{
			switch (pixelScaler)
			{
		...
		/// lifetime yourself, but as a single call, it's very easy to use.
		/// </remarks>
		public Image32 Scale3x(PixelScaler pixelScaler, bool includeAlpha = true)
		{
			switch (pixelScaler)
			{
				case PixelScaler.NearestNeighbor:
		...
		/// lifetime yourself, but as a single call, it's very easy to use.
		/// </remarks>
		public Image32 Scale4x(PixelScaler pixelScaler, bool includeAlpha = true)
		{
			switch (pixelScaler)
			{
				case PixelScaler.NearestNeighbor:
		...
		public void Blit(Image32 srcImage, int srcX, int srcY, int destX, int destY, int width, int height,
			BlitFlags blitFlags = default, Color32 color = default)
		{
			if (Object.ReferenceEquals(srcImage, null))
				throw new ArgumentNullException(nameof(srcImage), "Source image for a blit cannot be null.");
			...
			if ((blitFlags & BlitFlags.ModeMask) == BlitFlags.Copy
				&& !Object.ReferenceEquals(srcImage, this))
			{
			...
			FastUnsafeBlit32(srcImage, srcX, srcY, destX, destY, width, height, blitFlags, color);
		}
			...
			BlitFlags blitFlags = default, Color32 color = default)
		{
			...
			// General case:  Every other blit type.
			FastUnsafeBlit24(srcImage, srcX, srcY, destX, destY, width, height, blitFlags, color);
		}
		...
		/// will be ignored).
		/// </summary>
		/// <param name="srcImage">The source image to copy from.  Each pixel will be promoted to 32-bit RGBA via the image's palette.</param>
		/// <param name="srcX">The X coordinate of the top-left corner in the source image to start copying from.</param>
		/// <param name="srcY">The Y coordinate of the top-left corner in the source image to start copying from.</param>
		/// <param name="destX">The X coordinate of the top-left corner in the destination image to start copying to.</param>
		/// <param name="destY">The Y coordinate of the top-left corner in the destination image to start copying to.</param>
		...
		/// </summary>
			...
			[MethodImpl(MethodImplOptions.AggressiveOptimization)]
#endif
		private void FastUnsafeCopyBlit(Image32 srcImage, int srcX, int srcY, int destX, int destY, int width, int height, bool flipVert)
		{
			unsafe
			{
				fixed (Color32* destBase = Data)
		...
		[MethodImpl(MethodImplOptions.AggressiveOptimization)]
#endif
		private void FastUnsafeCopyBlitFlipHorz(Image32 srcImage, int srcX, int srcY, int destX, int destY, int width, int height, bool flipVert)
		{
			unsafe
			{
				fixed (Color32* destBase = Data)
			...
			unsafe
			{
				...
				fixed (Color32* srcBase = srcImage.Data)
				{
					InternalAlgorithms.SetupBlit(new Vector2i(width, height), blitFlags,
						...
						new Vector2i(destX, destY), Size, destBase, out Color32* dest, out int destStep, out int destSkip);
					...
					do
					{
						...
						switch (mode)
						{
							...
							case (int)BlitFlags.Alpha:
								for (; src != end; src += srcStep, dest += destStep)
									*dest = Color32.Over(over: *src, under: *dest);
								break;
							case (int)BlitFlags.AlphaPM:
								for (; src != end; src += srcStep, dest += destStep)
									*dest = Color32.OverPM(over: *src, under: *dest);
								break;
							case (int)BlitFlags.BlackAlpha:
								for (; src != end; src += srcStep, dest += destStep)
									*dest = Color32.BlackOver(src->A, under: *dest);
								break;
							case (int)BlitFlags.BlackAlphaPM:
								for (; src != end; src += srcStep, dest += destStep)
									*dest = Color32.BlackOverPM(src->A, under: *dest);
								break;
							case (int)BlitFlags.WhiteAlpha:
								for (; src != end; src += srcStep, dest += destStep)
									*dest = Color32.WhiteOver(src->A, under: *dest);
								break;
				...
				fixed (Color24* srcBase = srcImage.Data)
				{
					...
					do
					{
						...
						switch (mode)
						{
							...
							case (int)BlitFlags.Color:
								for (; src != end; src += srcStep, dest += destStep)
									*dest = ((Color32)(*src)).Scale(color);
								break;
							case (int)BlitFlags.ColorTransparent:
								for (; src != end; src += srcStep, dest += destStep)
								{
									Color32 c = ((Color32)(*src)).Scale(color);
									if (c.A != 0)
										*dest = c;
								}
								break;
							case (int)BlitFlags.ColorAlpha:
								for (; src != end; src += srcStep, dest += destStep)
									*dest = Color32.Over(over: ((Color32)(*src)).Scale(color), under: *dest);
								break;
							case (int)BlitFlags.ColorAlphaPM:
								for (; src != end; src += srcStep, dest += destStep)
									*dest = Color32.OverPM(over: ((Color32)(*src)).Scale(color), under: *dest);
								break;
						}
			...
			int width, int height, BlitFlags blitFlags, Color32 color)
		{
			...
			// Since we don't know what values the srcImage's pixels will take, and
			// the palette *could* be shorter than 256 entries, we reference the palette
			// using a span instead of a pointer, which will ensure that the compiler
			// emits bounds-checking for its lookups, and correctly throws exceptions
			// if the image references a nonexistent palette entry.
			ReadOnlySpan<Color32> palette = srcImage.Palette;
				...
				fixed (byte* srcBase = srcImage.Data)
				{
					...
					do
					{
						...
						switch (mode)
						{
							...
							case (int)BlitFlags.Alpha:
								for (; src != end; src += srcStep, dest += destStep)
									*dest = Color32.Over(over: palette[*src], under: *dest);
								break;
							case (int)BlitFlags.AlphaPM:
								for (; src != end; src += srcStep, dest += destStep)
									*dest = Color32.OverPM(over: palette[*src], under: *dest);
								break;
							case (int)BlitFlags.BlackAlpha:
								for (; src != end; src += srcStep, dest += destStep)
									*dest = Color32.BlackOver(palette[*src].A, under: *dest);
								break;
							case (int)BlitFlags.BlackAlphaPM:
								for (; src != end; src += srcStep, dest += destStep)
									*dest = Color32.BlackOverPM(palette[*src].A, under: *dest);
								break;
							case (int)BlitFlags.WhiteAlpha:
								for (; src != end; src += srcStep, dest += destStep)
									*dest = Color32.WhiteOver(palette[*src].A, under: *dest);
								break;
							case (int)BlitFlags.WhiteAlphaPM:
								for (; src != end; src += srcStep, dest += destStep)
									*dest = Color32.WhiteOverPM(palette[*src].A, under: *dest);
								break;
							case (int)BlitFlags.Color:
								for (; src != end; src += srcStep, dest += destStep)
									*dest = palette[*src].Scale(color);
								break;
							case (int)BlitFlags.ColorTransparent:
								for (; src != end; src += srcStep, dest += destStep)
								{
									Color32 c = palette[*src].Scale(color);
									if (c.A != 0)
										*dest = c;
								}
								break;
							case (int)BlitFlags.ColorAlpha:
								for (; src != end; src += srcStep, dest += destStep)
									*dest = Color32.Over(over: palette[*src].Scale(color), under: *dest);
								break;
							case (int)BlitFlags.ColorAlphaPM:
								for (; src != end; src += srcStep, dest += destStep)
									*dest = Color32.OverPM(over: palette[*src].Scale(color), under: *dest);
								break;
						}
		...
		public void Silhouette(Color32 color)
		{
			unsafe
			{
				fixed (Color32* imageBase = Data)
				{
					...
					while (ptr < end)
					{
						*ptr = new Color32(color.R, color.G, color.B, ptr->A);
						ptr++;
					}
				}
			}
		...
		/// </summary>
		/// <param name="color">The silhouette color.</param>
		public void SilhouettePM(Color32 color)
		{
			unsafe
			{
				fixed (Color32* imageBase = Data)
				{
					...
					while (ptr < end)
					{
						byte a = ptr->A;
						*ptr = new Color32(
							Color32.Div255(color.R * a),
							Color32.Div255(color.G * a),
							Color32.Div255(color.B * a), a);
						ptr++;
					}
				}
			}
			...
			unsafe
			{
				fixed (Color32* destBase = Data)
				{
					...
					switch (blitFlags & BlitFlags.ModeMask)
					{
						...
						case BlitFlags.AlphaPM:
							if (drawColor.A == 0)
								return;
							drawMode = drawColor.A == 255 ? BlitFlags.Copy : BlitFlags.AlphaPM;
							break;
					}
			...
			BlitFlags blitFlags = BlitFlags.Copy)
		{
			...
			switch (blitFlags & BlitFlags.ModeMask)
			{
				...
				case BlitFlags.AlphaPM:
					if (color.A == 0)
						return;
					drawMode = color.A == 255 ? BlitFlags.Copy : BlitFlags.AlphaPM;
					break;
			}
			...
			Color32 color, BlitFlags blitFlags = BlitFlags.ColorAlpha)
		{
			...
			DrawText(point, positionedGlyphs, color, blitFlags);
			return result;
		}
		...
		/// <param name="color">The color of the text.</param>
		/// <param name="blitFlags">Which blit mode to use when drawing each glyph.</param>
		public void DrawText(Vector2d point, IEnumerable<PositionedGlyph> glyphs,
			Color32 color, BlitFlags blitFlags = BlitFlags.ColorAlpha)
		{
		...
		public Image32 RoundBlur(double strength = 1.0)
		...
		public int MeasureContentStartX(Rect rect, byte alphaCutoff = 0)
		{
			...
			{
				if (!IsColumnTransparent(x, rect.Y, rect.Height, alphaCutoff))
					return x;
			}
		...
		public int MeasureContentStartY(Rect rect, byte alphaCutoff = 0)
		{
			...
			{
				if (!IsRowTransparent(rect.X, y, rect.Width, alphaCutoff))
					return y;
			}
		...
		public int MeasureContentWidth(Rect rect, byte alphaCutoff = 0)
		{
			...
			{
				if (!IsColumnTransparent(rect.X + width - 1, rect.Y, rect.Height, alphaCutoff))
					return width;
			}
			return 0;
		}
		...
		public int MeasureContentHeight(Rect rect, byte alphaCutoff = 0)
		{
			...
			{
				if (!IsRowTransparent(rect.X, rect.Y + height - 1, rect.Width, alphaCutoff))
					return height;
			}
			return 0;
		}
