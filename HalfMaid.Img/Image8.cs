...
namespace HalfMaid.Img
{
	...
	public sealed class Image8 : IImage<byte>
	{
		...
		{
			Image8? image = LoadFromBytes(data, filenameIfKnown, imageFormat);
			if (image == null)
		...
		public static Image8? LoadEmbeddedResource(Assembly assembly, string name)
		{
			byte[] bytes;
		...
		public static Image8? LoadFromBytes(ReadOnlySpan<byte> data, string? filenameIfKnown = null,
			ImageFormat imageFormat = default)
		{
		...
		/// <exception cref="ArgumentException">Raised if the new image size is illegal.</exception>
		void IImage.ResampleToFit(Vector2i containerSize, FitMode fitMode, ResampleMode mode)
			=> ResizeToFit(containerSize, fitMode);
		...
		/// be taken as a simple cubic B-spline.</param>
		/// <exception cref="ArgumentException">Raised if the new image size is illegal.</exception>
		void IImage.Resample(int? width, int? height, ResampleMode mode)
			=> Resize(CalculateResize(Size, width, height));
		...
		/// be taken as a simple cubic B-spline.</param>
		/// <exception cref="ArgumentException">Raised if the new image size is illegal.</exception>
		void IImage.Resample(Vector2i newSize, ResampleMode mode)
			=> Resize(newSize);
		...
		[MethodImpl(MethodImplOptions.AggressiveOptimization)]
#endif
		private void FastUnsafeCopyBlit(Image8 srcImage, int srcX, int srcY, int destX, int destY, int width, int height, bool flipVert)
		{
			unsafe
			{
				fixed (byte* destBase = Data)
		...
		[MethodImpl(MethodImplOptions.AggressiveOptimization)]
#endif
		private void FastUnsafeCopyBlitFlipHorz(Image8 srcImage, int srcX, int srcY, int destX, int destY, int width, int height, bool flipVert)
		{
			unsafe
			{
				fixed (byte* destBase = Data)
			...
			unsafe
			{
				...
				fixed (byte* srcBase = srcImage.Data)
				{
					InternalAlgorithms.SetupBlit(new Vector2i(width, height), blitFlags,
						...
						new Vector2i(destX, destY), Size, destBase, out byte* dest, out int destStep, out int destSkip);
		...
		public Rect MeasureContent(Rect rect, byte cutoff = 0)
		{
			...
			int width = MeasureContentWidth(new Rect(x, y, rect.Width - dx, rect.Height - dy), cutoff);
			int height = MeasureContentHeight(new Rect(x, y, rect.Width - dx, rect.Height - dy), cutoff);
