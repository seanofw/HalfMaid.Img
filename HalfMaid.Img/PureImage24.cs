...
namespace HalfMaid.Img
{
	...
	public readonly struct PureImage24 : IPureImage<Color24>
	{
		...
		private readonly Image24 _image;
		...
		[Pure]
		public Color24[] Data => _image.Data;
		...
		/// </summary>
		public static PureImage24 Empty { get; } = new PureImage24(new Image24(0, 0));
		...
		/// for simple cases.  Reading a pixel outside the image will return 'Black'.
		/// </summary>
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public PureImage24(Image24 image)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static implicit operator PureImage24(Image24 image)
			=> new PureImage24(image);
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static explicit operator Image24(PureImage24 pureImage)
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public PureImage24(string filename, ImageFormat imageFormat = default)
			: this(new Image24(filename, imageFormat))
		{
		}
		...
		/// <param name="imageFormat">The file format of that file, if known.</param>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public PureImage24(ReadOnlySpan<byte> data, string? filenameIfKnown = null,
			ImageFormat imageFormat = default)
			: this(new Image24(data, filenameIfKnown, imageFormat))
		{
		}
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public PureImage24(Vector2i size)
			: this(new Image24(size))
		{
		}
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public PureImage24(Vector2i size, Color24 fillColor)
			: this(new Image24(size, fillColor))
		{
		}
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public PureImage24(int width, int height, ReadOnlySpan<byte> srcData, ReadOnlySpan<Color32> palette)
			: this(new Image24(width, height, srcData, palette))
		{
		}
		...
		/// <param name="height">The height of the new image, in pixels.</param>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public PureImage24(int width, int height)
			: this(new Image24(width, height))
		{
		}
		...
		/// <param name="fillColor">The color for all of the new pixels in the image.</param>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public PureImage24(int width, int height, Color24 fillColor)
			: this(new Image24(width, height, fillColor))
		{
		}
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public PureImage24(int width, int height, Color24[] data)
			: this(new Image24(width, height, data))
		{
		}
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public PureImage24(int width, int height, ReadOnlySpan<byte> rawData)
			: this(new Image24(width, height, rawData))
		{
		}
		...
		/// <exception cref="ArgumentException">Thrown if rawData is too short for the image dimensions.</exception>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public unsafe PureImage24(int width, int height, byte* rawData, int rawDataLength)
			: this(new Image24(width, height, rawData, rawDataLength))
		{
		}
		...
		/// <exception cref="ArgumentException">Thrown if rawData is too short for the image dimensions.</exception>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public PureImage24(int width, int height, ReadOnlySpan<Color24> data)
			: this(new Image24(width, height, data))
		{
		}
		...
		/// array.</param>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public unsafe PureImage24(int width, int height, Color24* data, int dataLength)
			: this(new Image24(width, height, data, dataLength))
		{
		}
		...
		[Pure]
		public static PureImage24 LoadFile(string filename, ImageFormat imageFormat = default)
			=> Image24.LoadFile(filename, imageFormat) ?? Empty;
		...
		[Pure]
		public PureImage24 ResizeToFit(Vector2i containerSize, FitMode fitMode = FitMode.Fit)
		...
		/// <returns>A copy of this image, resized to fit the container.</returns>
		[Pure]
		public PureImage24 Resize(Vector2i imageSize)
		...
		/// <returns>A copy of this image, resized to fit the container.</returns>
		[Pure]
		public PureImage24 Resize(int newWidth, int newHeight)
		{
			Image24 result = new Image24(newWidth, newHeight);
			...
			unsafe
			{
				...
				fixed (Color24* srcBase = Data)
				{
					for (int destY = 0, srcYScaled = 0; destY < newHeight; destY++, srcYScaled += yStep)
					{
						...
						Color24* srcRow = srcBase + srcY * Width;
						Color24* dest = destBase + destY * newWidth;
						Color24* destEnd = dest + newWidth;
		...
		/// <exception cref="ArgumentException">Raised if the new image size is illegal.</exception>
		[Pure]
		public PureImage24 ResampleToFit(Vector2i containerSize, FitMode fitMode = FitMode.Fit, ResampleMode mode = ResampleMode.BSpline)
		...
		/// <exception cref="ArgumentException">Raised if the new image size is illegal.</exception>
		[Pure]
		public PureImage24 Resample(Vector2i newSize, ResampleMode mode = ResampleMode.BSpline)
		{
		...
		/// </summary>
		[Pure]
		public PureImage24 Extract(int x, int y, int width, int height)
		...
		/// </summary>
		[Pure]
		public PureImage24 Crop(int x, int y, int width, int height)
			=> _image.Extract(x, y, width, height);
		...
		[Pure]
		public PureImage24 Pad(int left = 0, int top = 0, int right = 0, int bottom = 0, Color24 fillColor = default)
		{
			Image24 image = new Image24(Width + left + right, Height + top + bottom, fillColor);
		...
		/// <remarks>
		/// This Blit() does *not* perform the update in-place, and is typically slower than Image24.Blit()
		/// by a lot.  This is usually not the method you want unless regularity (or piping)
		/// matters more to you than performance.
		/// </remarks>
		[Pure]
		public PureImage24 Blit(Image24 srcImage, int srcX, int srcY, int destX, int destY, int width, int height,
			BlitFlags blitFlags = default, Color24 color = default)
		{
			Image24 copy = _image.Clone();
		...
		/// </remarks>
		[Pure]
		public PureImage24 Blit(Image32 srcImage, int srcX, int srcY, int destX, int destY, int width, int height,
			BlitFlags blitFlags = default, Color24 color = default)
		{
			Image24 copy = _image.Clone();
			copy.Blit(srcImage, srcX, srcY, destX, destY, width, height, blitFlags, color);
		...
		/// </remarks>
		[Pure]
		public PureImage24 Blit(Image8 srcImage, int srcX, int srcY, int destX, int destY, int width, int height,
			BlitFlags blitFlags = default, Color24 color = default)
		{
			Image24 copy = _image.Clone();
			copy.Blit(srcImage, srcX, srcY, destX, destY, width, height, blitFlags, color);
		...
		[Pure]
		public PureImage24 PatternBlit(Image24 srcImage, Rect srcRect, Rect destRect)
		{
			Image24 copy = _image.Clone();
		...
		public PureImage24 FlipVert()
		{
			...
			unsafe
			{
				...
				fixed (Color24* destBase = result.Data)
				{
					Color24* srcRow = srcBase;
					Color24* destRow = destBase + Width * (Height - 1);
		...
		public PureImage24 Rotate90()
		{
			...
			unsafe
			{
				...
				fixed (Color24* destBase = result.Data)
				{
					...
					{
						Color24* src = srcBase + x + oldWidth * (oldHeight - 1);
						Color24* dest = destBase + x * newWidth;
		...
		[Pure]
		public PureImage24 Rotate90CCW()
		{
			// The new dimensions.
			int oldWidth = Width, oldHeight = Height;
			int newWidth = Height, newHeight = Width;
			...
			unsafe
			{
				...
				fixed (Color24* destBase = result.Data)
				{
					...
					{
						Color24* src = srcBase + x * oldWidth;
						Color24* dest = destBase + x + newWidth * (newHeight - 1);
		...
		public PureImage24 Rotate180()
		{
			...
			unsafe
			{
				...
				{
					Color24* src = srcBase;
					Color24* dest = destBase + Width * Height;
					Color24* end = srcBase + Width * Height;
					while (src < end)
					{
						*--dest = *src++;
					}
		...
		[Pure]
		public PureImage24 Mix(Image24 other, double amount = 0.5)
		{
			Image24 copy = _image.Clone();
		...
		public PureImage24 Multiply(float r, float g, float b)
		{
			Image24 copy = _image.Clone();
			copy.Multiply(r, g, b);
			return copy;
		...
		/// </summary>
		/// <returns>A copy of this image, with every color value multiplied by the given scalars.</returns>
		public PureImage24 Multiply(double r, double g, double b)
		{
			Image24 copy = _image.Clone();
			copy.Multiply(r, g, b);
			return copy;
		...
		[Pure]
		public PureImage24 RemapColor(Color24 src, Color24 dest)
		{
			Image24 copy = _image.Clone();
		...
		[Pure]
		public PureImage24 RemapColor(Dictionary<Color24, Color24> dictionary)
		{
			Image24 copy = _image.Clone();
		...
		[Pure]
		public PureImage24 RemapColor(Matrix3 matrix)
		{
			Image24 copy = _image.Clone();
		...
		[Pure]
		public PureImage24 Gamma(double gamma)
		{
			Image24 copy = _image.Clone();
			copy.Gamma(gamma);
			return copy;
		...
		[Pure]
		public PureImage24 Gamma(double rAmount, double gAmount, double bAmount)
		{
			Image24 copy = _image.Clone();
		...
		[Pure]
		public PureImage24 Grayscale(bool useRelativeBrightness = true,
		...
		[Pure]
		public PureImage24 Desaturate(double amount,
		...
		[Pure]
		public PureImage24 Sepia(double amount = 0.125)
		{
			Image24 copy = _image.Clone();
			copy.Sepia(amount);
			return copy;
		...
		/// <returns>A copy of this image, inverted.</returns>
		[Pure]
		public PureImage24 Invert(ColorChannel channel)
		{
			Image24 copy = _image.Clone();
			copy.Invert(channel);
			return copy;
		...
		[Pure]
		public PureImage24 HueSaturationLightness(double deltaHue, double deltaSaturation, double deltaLightness)
		{
			Image24 copy = _image.Clone();
		...
		/// <returns>A copy of this image, with the given color adjustments applied.</returns>
		[Pure]
		public PureImage24 HueSaturationBrightness(double deltaHue, double deltaSaturation, double deltaBrightness)
		{
			Image24 copy = _image.Clone();
		...
		[Pure]
		public PureImage24 BrightnessContrast(double brightness, double contrast)
		{
			Image24 copy = _image.Clone();
		...
		/// <returns>A copy of this image, with the color adjustments applied.</returns>
		[Pure]
		public PureImage24 AdjustRange(double newMin, double newRange)
		{
			Image24 copy = _image.Clone();
		...
		/// <returns>A copy of this image, with the color adjustments applied.</returns>
		[Pure]
		public PureImage24 AdjustRange(double redNewMin, double redRange,
		...
		/// <returns>A copy of this image, with the color adjustments applied.</returns>
		[Pure]
		public PureImage24 RemapValues(ReadOnlySpan<byte> redLookup, ReadOnlySpan<byte> greenLookup,
			ReadOnlySpan<byte> blueLookup)
		{
			Image24 copy = _image.Clone();
			copy.RemapValues(redLookup, greenLookup, blueLookup);
			return copy;
		}
		...
		/// <returns>A copy of this image, with the color adjustments applied.</returns>
		public PureImage24 ColorTemperature(double temperature)
		{
			Image24 copy = _image.Clone();
		...
		/// <summary>
		/// Combine separate 8-bit RGBA channel images into a single 24-bit RGBA image.
		/// </summary>
		...
		[Pure]
		public static PureImage24 CombineChannels(Image8? red = null, Image8? green = null, Image8? blue = null)
			=> Image24.CombineChannels(red, green, blue);
		...
		[Pure]
		public PureImage24 SwapChannels(ColorChannel redChannel,
			ColorChannel greenChannel, ColorChannel blueChannel)
		{
			Image24 copy = _image.Clone();
			copy.SwapChannels(redChannel, greenChannel, blueChannel);
			return copy;
		}
		...
		[Pure]
		public static PureImage24 operator +(PureImage24 a, PureImage24 b)
			=> a._image + b._image;
		...
		[Pure]
		public static PureImage24 operator *(PureImage24 image, int scalar)
			=> image._image * scalar;
		...
		/// <returns>A new image where all components have been scaled.</returns>
		[Pure]
		public static PureImage24 operator *(PureImage24 image, double scalar)
			=> image._image * scalar;
		...
		[Pure]
		public static PureImage24 operator ~(PureImage24 image)
			=> ~image._image;
		...
		[Pure]
		public static PureImage24 operator <<(PureImage24 image, int amount)
			=> image._image << amount;
		...
		public Color24[] Quantize(int numColors, bool useOriginalColors = false)
			=> _image.Quantize(numColors, useOriginalColors);
		...
		public Image8 ToImage8(int numColors = 256, DitherMode ditherMode = default,
			...
			=> _image.ToImage8(numColors, ditherMode,
				useOriginalColors, useVisualWeighting);
		...
		/// <summary>
		/// Convert an Image24 to an Image32.
		/// </summary>
		/// <returns>A copy of this image.</returns>
		[Pure]
		...
		[Pure]
		public PureImage24 Fill(Color24 color)
			=> new PureImage24(Size, color);
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		[Pure]
		public PureImage24 FillRect(Rect rect, Color24 drawColor, BlitFlags blitFlags = BlitFlags.Copy)
		...
		/// <returns>A copy of the image that has had the given rectangle filled with color.</returns>
		[Pure]
		public PureImage24 FillRect(int x, int y, int width, int height, Color24 drawColor,
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public PureImage24 FillGradientRect(Rect rect, Color24 topLeft, Color24 topRight, Color24 bottomLeft, Color24 bottomRight,
			BlitFlags blitFlags = BlitFlags.Copy)
		...
		/// fully contained within the image.</param>
		/// <returns>A copy of the image, with a gradient rectangle drawn on it.</returns>
		public PureImage24 FillGradientRect(int x, int y, int width, int height,
		...
		/// rectangle is fully contained within the image.</param>
		[Pure]
		public PureImage24 DrawRect(Rect rect, Color24 color, int thickness = 1, BlitFlags blitFlags = BlitFlags.Copy)
		...
		/// rectangle is fully contained within the image.</param>
		[Pure]
		public PureImage24 DrawRect(int x, int y, int width, int height, Color24 color, int thickness = 1,
			BlitFlags blitFlags = BlitFlags.Copy)
		{
			Image24 copy = _image.Clone();
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		[Pure]
		public PureImage24 DrawLine(Vector2i p1, Vector2i p2, Color24 color, BlitFlags blitFlags = BlitFlags.Copy)
		...
		/// <returns>A copy of the image, with the line drawn on it.</returns>
		[Pure]
		public PureImage24 DrawLine(int x1, int y1, int x2, int y2, Color24 color, BlitFlags blitFlags = BlitFlags.Copy)
		{
			Image24 copy = _image.Clone();
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		[Pure]
		public PureImage24 DrawThickLine(int x1, int y1, int x2, int y2, double thickness,
		...
		[Pure]
		public PureImage24 DrawThickLine(Vector2i start, Vector2i end, double thickness,
			Color24 color, BlitFlags blitFlags = BlitFlags.Copy)
		{
			Image24 copy = _image.Clone();
		...
		/// <returns>A copy of the image, with a thick line drawn on it.</returns>
		[Pure]
		public PureImage24 DrawThickLine(Vector2d start, Vector2d end, double thickness,
			Color24 color, BlitFlags blitFlags = BlitFlags.Copy)
		{
			Image24 copy = _image.Clone();
			copy.DrawThickLine(start, end, thickness, color, blitFlags);
		...
		[Pure]
		public PureImage24 FillPolygon(IEnumerable<Vector2> points, Color24 color,
			BlitFlags blitFlags = BlitFlags.Copy)
		{
		...
		/// <returns>A copy of the image, with the given polygon drawn on top of it.</returns>
		[Pure]
		public PureImage24 FillPolygon(IEnumerable<Vector2d> points, Color24 color,
			BlitFlags blitFlags = BlitFlags.Copy)
		{
		...
		/// <returns>A copy of the image, with the given polygon drawn on top of it.</returns>
		[Pure]
		public PureImage24 FillPolygon(IEnumerable<Vector2i> points, Color24 color,
			BlitFlags blitFlags = BlitFlags.Copy)
		{
		...
		/// <returns>A copy of the image, with the given polygon drawn on top of it.</returns>
		[Pure]
		public PureImage24 FillPolygon(ReadOnlySpan<Vector2> points, Color24 color,
			BlitFlags blitFlags = BlitFlags.Copy)
		{
			Image24 copy = _image.Clone();
		...
		/// <returns>A copy of the image, with the given polygon drawn on top of it.</returns>
		[Pure]
		public PureImage24 FillPolygon(ReadOnlySpan<Vector2d> points, Color24 color,
			BlitFlags blitFlags = BlitFlags.Copy)
		{
			Image24 copy = _image.Clone();
			copy.FillPolygon(points, color, blitFlags);
		...
		/// <returns>A copy of the image, with the given polygon drawn on top of it.</returns>
		[Pure]
		public PureImage24 FillPolygon(ReadOnlySpan<Vector2i> points, Color24 color,
			BlitFlags blitFlags = BlitFlags.Copy)
		{
			Image24 copy = _image.Clone();
			copy.FillPolygon(points, color, blitFlags);
		...
		[Pure]
		public PureImage24 DrawBezier(Vector2 p1, Vector2 c1, Vector2 c2, Vector2 p2,
		...
		/// <param name="blitFlags">Which blit mode to use when drawing each glyph.</param>
		/// <returns>The new image with the text drawn on it.</returns>
		public PureImage24 DrawText(Vector2d point, ReadOnlySpan<char> text, Font font,
			Color32 color, BlitFlags blitFlags = BlitFlags.ColorAlpha)
		{
			Image24 clone = _image.Clone();
		...
		/// <param name="blitFlags">Which blit mode to use when drawing each glyph.</param>
		/// <returns>The new image with the text drawn on it.</returns>
		public PureImage24 DrawText(Vector2d point, IEnumerable<PositionedGlyph> glyphs,
			Color32 color, BlitFlags blitFlags = BlitFlags.ColorAlpha)
		{
			Image24 clone = _image.Clone();
		...
		public PureImage24 Emboss(double strength = 1.0)
		...
		public PureImage24 Sharpen(double strength = 1.0)
		...
		public PureImage24 EdgeDetect(double strength = 1.0, bool includeDiagonals = false)
		...
		public PureImage24 BoxBlur(double strength = 1.0)
		...
		public PureImage24 Convolve3x3(double[] kernel, double strength = 1.0, bool divideBySum = false)
		...
		/// <returns>A copy of the image, with the convolution kernel applied.</returns>
		[Pure]
		public PureImage24 ConvolveHorz(ReadOnlySpan<double> kernel, double strength = 1.0, bool divideBySum = false)
		...
		/// <returns>A copy of the image, with the convolution kernel applied.</returns>
		[Pure]
		public PureImage24 ConvolveVert(ReadOnlySpan<double> kernel, double strength = 1.0, bool divideBySum = false)
		...
		/// <returns>A copy of the image, with the convolution kernel applied.</returns>
		[Pure]
		public PureImage24 Convolve(ReadOnlySpan<double> kernel, int kernelWidth, int kernelHeight,
		...
		[Pure]
		public PureImage24 GaussianBlur(double radius, double? sigma = null, double strength = 1.0)
		...
		public int MeasureContentWidth(Rect rect, Color24 transparentColor)
			=> _image.MeasureContentWidth(rect, transparentColor);
		...
		public int MeasureContentHeight(Rect rect, Color24 transparentColor)
			=> _image.MeasureContentHeight(rect, transparentColor);
		...
		public bool IsRectTransparent(Rect rect, Color24 transparentColor)
			=> _image.IsRectTransparent(rect, transparentColor);
		...
		public bool IsRowTransparent(int x, int y, int width, Color24 transparentColor)
			=> _image.IsRowTransparent(x, y, width, transparentColor);
		...
		public bool IsColumnTransparent(int x, int y, int height, Color24 transparentColor)
			=> _image.IsColumnTransparent(x, y, height, transparentColor);
		...
		public override bool Equals(object? obj)
			=> obj is PureImage24 other && Equals(other)
				|| obj is Image24 other2 && _image.Equals(other2);
		...
		/// <returns>True if the other image is an identical image to this image, false otherwise.</returns>
		[Pure]
		public bool Equals(Image24 other)
			=> _image.Equals(other);
		...
		[Pure]
		public static bool operator ==(PureImage24 a, PureImage24 b)
			=> a.Equals(b);
